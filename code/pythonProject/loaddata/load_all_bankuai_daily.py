...
def insert_data(table_name, data, index_name):
    """
    插入数据到指定表
    :param table_name: 表名
    :param data: 数据 DataFrame
...
for index, row in industry_index_df.iterrows():
    ...
    index_name = row['name']
    try:
        data = get_data(ts_code, five_years_ago, today)
        if not data.empty:
            data = calculate_indicators(data)
            insert_data('industry_sector_daily', data, index_name)
        else:
            print(f"未获取到 {ts_code} 的行业指数数据。")
...
for index, row in concept_index_df.iterrows():
    ...
    try:
        ...
        if not data.empty:
            data = calculate_indicators(data)
            insert_data('concept_sector_daily', data, index_name)
        else:
            print(f"未获取到 {ts_code} 的概念指数数据。")
    except Exception as e:
